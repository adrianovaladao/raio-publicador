    ...
    <div className="card" style={{ marginTop: 16 }}>
      ...
      {!loaded ? (
        ...
        <>
          ...
          {groups.map((group, gi) => (
            <div key={group}>
              <div style={{ padding: "16px 22px 8px", fontSize: 11, fontWeight: 600, color: "var(--stone)", textTransform: "uppercase", letterSpacing: "0.06em", borderTop: gi > 0 ? "1px solid var(--line)" : undefined }}>
                {group}
              </div>
              {(Object.entries(NOTIF_LABELS) as [NotifType, { title: string; desc: string; group: string }][])
                .filter(([, v]) => v.group === group)
                .map(([type, meta]) => (
                  <div key={type} style={{ display: "flex", alignItems: "center", padding: "14px 22px", gap: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
