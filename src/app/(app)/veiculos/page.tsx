...
type VehicleItem = { id: string; name: string; domain: string; site?: string | null; location?: string | null; cat: string; tier: string; reach: number; logoUrl?: string | null; avgPublicationHours?: number | null };
...
export default function VeiculosPage() {
  ...
  useEffect(() => {
    fetch("/api/vehicles")
      .then(r => r.json())
      .then((data: { id: string; name: string; domain: string; site?: string | null; location?: string | null; category: string; tier: string; reach: number; logoUrl?: string | null; avgPublicationHours?: number | null }[]) => {
        setVehicles(data.map(v => ({ id: v.id, name: v.name, domain: v.domain, site: v.site, location: v.location, cat: v.category, tier: v.tier, reach: v.reach, logoUrl: v.logoUrl, avgPublicationHours: v.avgPublicationHours })));
      })
      .catch(() => {});
  }, []);
                      ...
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ position: "relative", display: "inline-flex" }} className="clock-wrap">
                          ...
                          <span className="clock-tip">
                            Tempo médio de publicação: {fmtAvgTime(v.avgPublicationHours ?? TIER_AVG_HOURS[v.tier] ?? 48)}
                          </span>
                        </span>
                      </span>
                    </td>
