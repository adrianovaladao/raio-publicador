    ...
    <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "var(--cream, #FAF9F7)", fontFamily: "var(--font-dm-sans, sans-serif)", padding: 24, zoom: 1.15 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: "48px 40px", maxWidth: 440, width: "100%", boxShadow: "0 2px 24px rgba(0,0,0,0.07)", textAlign: "center" }}>
        ...
        <div style={{ margin: "0 auto 28px", display: "flex", justifyContent: "center" }}>
          <RaioLockup height={36} variant="dark" />
        </div>
        ...
        {status === "error" && (
          <>
            <h2 style={{ margin: "0 0 12px", fontSize: 20 }}>Convite inválido</h2>
            <p style={{ color: "#777", margin: "0 0 8px", fontSize: 14 }}>{err}</p>
            {(err.includes("expirou") || err.includes("utilizado")) && (
              <p style={{ color: "#aaa", margin: "0 0 24px", fontSize: 12 }}>
        ...
        {status === "done" && (
          <>
            ...
            <p style={{ color: "#777", fontSize: 14 }}>Bem-vindo ao Raio Publicador. Redirecionando…</p>
          </>
        )}
