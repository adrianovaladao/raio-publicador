...
import { Plus, Trash2, RefreshCw, Tag, Copy, Check, ChevronUp, ChevronDown, ChevronsUpDown, Search, X, Archive, ArchiveRestore } from "lucide-react";
...
export default function VouchersAdminPage() {
  ...
  async function handleArchiveAllUsed() {
    const used = vouchers.filter(v => !v.archivedAt && v.usedCount >= v.maxUses);
    for (const v of used) {
  ...
  const activeVouchers   = useMemo(() => vouchers.filter(v => !v.archivedAt), [vouchers]);
  const archivedVouchers = useMemo(() => vouchers.filter(v => !!v.archivedAt), [vouchers]);
  const usedNotArchived  = useMemo(() => activeVouchers.filter(v => v.usedCount >= v.maxUses).length, [activeVouchers]);

  const base = tab === "active" ? activeVouchers : archivedVouchers;
  ...
  const sorted = useMemo(() => {
    return [...filtered].sort((a, b) => {
      ...
      if (sortKey === "credits" || sortKey === "usedCount") {
        ...
        av = (a[sortKey as keyof Voucher] ?? "").toString().toLowerCase();
        bv = (b[sortKey as keyof Voucher] ?? "").toString().toLowerCase();
      }
    ...
    {confirmDelete && (
      <div className="overlay" onClick={() => setConfirmDelete(null)}>
        <div className="modal" style={{ maxWidth: 420 }} onClick={e => e.stopPropagation()}>
          <div className="m-head"><h3>Excluir <em>voucher</em></h3></div>
          <div className="m-body" style={{ paddingBottom: 24 }}>
            <p style={{ fontSize: 14, color: "var(--stone)", marginBottom: 24 }}>
              Tem certeza que deseja excluir permanentemente o voucher <b style={{ fontFamily: "monospace", color: "var(--ink)" }}>{confirmDelete.code}</b>? Esta ação não pode ser desfeita.
            </p>
    ...
    <div className="content scroll">
      <div className="content-inner">
          ...
          <form onSubmit={handleCreate}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 100px 1fr 1fr auto", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
              <div className="field" style={{ marginBottom: 0 }}>
                <label>Código *</label>
                <input className="input" required placeholder="EX: RAIO-WELCOME" value={form.code}
                  onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))}
                  style={{ fontFamily: "monospace", letterSpacing: "0.05em" }} />
              </div>
              <div className="field" style={{ marginBottom: 0 }}>
                <label>Créditos *</label>
                <input className="input" type="number" min={1} required value={form.credits}
                  onChange={e => setForm(f => ({ ...f, credits: e.target.value }))} />
              </div>
              <div className="field" style={{ marginBottom: 0 }}>
                <label>Usos máx.</label>
                <input className="input" type="number" min={1} value={form.maxUses}
                  onChange={e => setForm(f => ({ ...f, maxUses: e.target.value }))} />
              </div>
              <div className="field" style={{ marginBottom: 0 }}>
                <label>Descrição interna</label>
                <input className="input" placeholder="Ex: Prospect Folhapress" value={form.description}
                  onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
              </div>
              <div className="field" style={{ marginBottom: 0 }}>
                <label>Validade</label>
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                  {[{ label: "7d", days: 7 }, { label: "15d", days: 15 }, { label: "30d", days: 30 }, { label: "60d", days: 60 }, { label: "∞", days: 0 }].map(({ label, days }) => {
                    ...
                    return (
                      <button key={label} type="button" onClick={() => setForm(f => ({ ...f, expiresAt: val }))}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
          {(["active", "archived"] as Tab[]).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === "active" ? `Ativos (${activeVouchers.length})` : `Arquivados (${archivedVouchers.length})`}
            </button>
          ))}
          {tab === "active" && usedNotArchived > 0 && (
            <button
              onClick={handleArchiveAllUsed}
              className="btn btn-ghost btn-sm"
              style={{ marginLeft: "auto", gap: 5, color: "var(--stone)" }}
              title="Arquivar todos os vouchers já esgotados">
              <Archive size={13} /> Arquivar esgotados ({usedNotArchived})
            </button>
          )}
        </div>
        ...
        <div style={{ marginBottom: 16, position: "relative" }}>
          ...
          <input className="input" placeholder="Buscar por código ou descrição…" value={search}
        ...
        <div className="card">
          {loading ? (
            ...
            <div className="card empty" style={{ flexDirection: "column", gap: 10 }}>
              ...
              <div className="muted">
                {search
                  ? `Nenhum voucher encontrado para "${search}".`
                  : tab === "archived"
                    ? "Nenhum voucher arquivado."
            ...
            <table className="tbl admin-vouchers-tbl">
                  ...
                  {(["code", "credits", "usedCount", "description", "expiresAt", "createdAt"] as SortKey[]).map(col => {
                    ...
                    return (
                      <th key={col} style={{ cursor: "pointer", userSelect: "none", whiteSpace: "nowrap" }} onClick={() => toggleSort(col)}>
                        <span style={{ display: "inline-flex", alignItems: "center" }}>{labels[col]}<SortIcon col={col} /></span>
                      </th>
                    );
                  })}
              ...
              <tbody>
                {sorted.map(v => (
                  <tr key={v.id} style={{ opacity: v.archivedAt ? 0.65 : 1 }}>
                    <td className="title-cell">
                      <button onClick={() => copyCode(v.code)} title="Copiar código"
                    ...
                    <td style={{ textAlign: "center" }}>
                      <div style={{ display: "inline-flex", gap: 4 }}>
                        ...
                        <button
                          ...
                          onClick={() => handleArchive(v.id, !v.archivedAt)}
                          disabled={archiving === v.id}
                          title={v.archivedAt ? "Desarquivar" : "Arquivar"}>
                          {archiving === v.id
                            ? <RefreshCw size={13} />
                            : v.archivedAt
