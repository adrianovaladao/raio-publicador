...
// Used by the AppShell to detect editor/reviewer sessions.
export async function GET() {
  ...
  const member = await getPrisma().teamMember.findUnique({
    ...
    select: { role: true, ownerId: true, name: true, status: true, brands: { select: { brandId: true } } },
  });
  ...
  return NextResponse.json({
    ...
    name:     member.name,
    brandIds: member.brands.map(b => b.brandId),
  });
}
