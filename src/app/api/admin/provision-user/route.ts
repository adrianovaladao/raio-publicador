...
export async function POST(req: Request) {
  ...
  // Marca email como verificado no Clerk
  try {
    const clerk = await clerkClient();
    const user = await clerk.users.getUser(userId);
    const emailId = user.emailAddresses[0]?.id;
    if (emailId) {
      // Clerk não permite forçar verificação via SDK — já foi feito no dashboard
  ...
  // Aplica voucher se informado
  if (voucherCode) {
    const code = voucherCode.trim().toUpperCase();
    const voucher = await prisma.voucher.findUnique({ where: { code } });

    if (!voucher) {
      return NextResponse.json({ error: `Voucher ${code} não encontrado.`, results }, { status: 404 });
    }
    if (voucher.expiresAt && voucher.expiresAt < new Date()) {
      return NextResponse.json({ error: "Voucher expirado.", results }, { status: 400 });
    }
    if (voucher.usedCount >= voucher.maxUses) {
      return NextResponse.json({ error: "Voucher esgotado.", results }, { status: 400 });
    }
      ...
      await prisma.$transaction([
        ...
        existingSub
          ? prisma.subscription.update({
              ...
              data: { creditsTotal: { increment: voucher.credits } },
            })
