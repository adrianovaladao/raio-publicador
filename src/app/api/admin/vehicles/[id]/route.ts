...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { name, domain, site, location, category, tier, reach, logoUrl, avgPublicationHours } = await req.json();

  try {
    const vehicle = await getPrisma().vehicle.update({
      where: { id },
      data: { name, domain, site: site ?? null, location: location ?? null, category, tier, reach: Number(reach), logoUrl: logoUrl ?? null, avgPublicationHours: avgPublicationHours != null ? Number(avgPublicationHours) : null },
    });
