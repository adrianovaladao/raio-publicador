...
export async function POST(req: Request) {
  ...
  const { name, domain, site, location, category, tier, reach, logoUrl, avgPublishDays } = await req.json();
  ...
  try {
    ...
    const vehicle = await prisma.vehicle.create({
      data: { name, domain, site: site ?? null, location: location ?? null, category, tier, reach: Number(reach), logoUrl: logoUrl ?? null, avgPublishDays: avgPublishDays != null ? Number(avgPublishDays) : null },
    });
