...
export const DEFAULT_PREFS: Record<NotifType, { inApp: boolean; email: boolean }> = {
  ...
  plan_renewed:             { inApp: true, email: true },
  plan_changed:             { inApp: true, email: true },
  subscription_cancelled:   { inApp: true, email: true },
  subscription_reactivated: { inApp: true, email: true },
  member_joined:            { inApp: true, email: true },
  vehicle_added:            { inApp: true, email: true },
};
...
export const NOTIF_LABELS: Record<NotifType, { title: string; desc: string; group: string }> = {
  ...
  release_rejected:         { group: "Releases", title: "Release cancelado",            desc: "Quando um release é cancelado pela equipe." },
